...
class Sim:
    ...
    def __init__ (self, scenario, params, folder):
        ...
        self.Outputs = ['day', 'currentPop', 'popFromStart', 'numHouseholds', 'averageHouseholdSize', 'marriageTally', 
                        ...
                        'mobilityReduction_A6', 'mobilityReduction_A7', 'mobilityReduction_A8', 'mobilityReduction_A9', 'totalAttendances', 'cumulatedAbsences']
...
#        self.medianIsolationRate = np.median(behaviouralIsolationRates)
    ...
    def createInfoNetworks_R(self):
        ...
        for z in range(int(self.p['incomeClasses'])):
            a += (1.0/float(self.p['incomeClasses']))*math.pow(self.p['classContactBias'], z)
        for i in range(int(self.p['interactionAgeClasses'])): # 
            meanFriends = sum(self.contacts[i])*self.p['friendsContactsRatio']
            # Given these total contacts for the class has a whole, find the contacts fro this age group for each income quintile class
            lowClassContacts = meanFriends/a
            friendsByIncome = []
            for j in range(int(self.p['incomeClasses'])):
...
#                notAvailable = []
#                for household in agent.house.infoNetwork.nodes():
#                    notAvailable.extend(household.occupants)
                ...
                if len(contactsGroup) > 0:
                    ...
                    ageDistances = [abs(x.age-agent.age) for x in contactsGroup]
                    ...
                    totalExps = [(h+c+d-f) for h, c, d, f in zip(ageExps, classExps, distanceExps, friendsExp)]
...
#            randomContact.socialContacts.add_edge(randomContact, randomAgent, weight = weightLink)
    ...
    def commutingProcess(self):
        commutingAgents = [x for x in self.pop.livingPeople if x.age > 17 and x.age < 70]
        for agent in commutingAgents:
            if np.random.random() < self.p['commutingProb']:
                ...
                weights = [x/np.power(y, 2.0) for x, y in zip(pop, dist)]
...
#                    print ''
                ...
                # In this case, the probability of an age/income class being selected depends on the actual venue's attendants
                weightsAges = []
                for j in range(int(self.p['interactionAgeClasses'])):
                    deltaContacts = []
                ...
                if sum(weightsAges) > 0:
                    ...
                    potentialVisitors = [x for x in agents if x.interactionAgeClass == ageClass if x not in venue.occupants]
                ...
                if len(potentialVisitors) > 0:
                    ...
                    if agent.hospitalized == False:
                        if self.p['behaviourModuleOn'] == True:
                            ...
                            if random.random() < probAttendance:
                                venue.occupants.append(agent)
                            else:
...
#            venue.occupants = [x for x in actualVisitors]
    ...
    def updateContactRates(self, day):
        ...
        for house in self.map.occupiedHouses:
            ...
            if day == 0:
                house.baseIsolationPropensities = [0]*26
            else:
            # Compute weighted average of neighbors' isolation rate in previous period
#            neighbors = [x for x in house.infoNetwork.neighbors(house)]
#            linksWeights = [house.infoNetwork[house][x]['weight'] for x in neighbors]
#            weights = [x/sum(linksWeights) for x in linksWeights]
#            socialIsolations = []
#            for i in range(int(self.p['isolationRatesNum'])):
#                isolationRates = [x.contactReductionRates[i] for x in neighbors]
#                socialIsolations.append(sum([w*r for w, r in zip(weights, isolationRates)]))
            
            # Network's events
            
                if self.newCasesRatio > 0:
                
                    hospitalizationEvents = [x for x in house.periodEvents if x.condition == 'hospitalized']
                    intubationEvents = [x for x in house.periodEvents if x.condition == 'intubated']
                    deathEvents = [x for x in house.periodEvents if x.condition == 'death'] 
                    
                    # Base household's isolation rates
                    highestExpectedCost = 0
                    for member in house.occupants:
                        ageIndex = -1
                        if member.age < 19:
                            ageIndex = 0
                        elif member.age >= 19 and member.age < 40:
                            ageIndex = 1
                        elif member.age >= 40 and member.age < 59:
                            ageIndex = 2
                        elif member.age >= 60 and member.age < 79:
                            ageIndex = 3
                        else:
                            ageIndex = 4
                        expectedCost = 0
                        
                        # Update probability of pandemic events based on events in network
                        
                        expectedCost = 0
                        for i in range(int(self.p['numConditions'])):
                            baseProb = max(self.probsConditionsByAge[ageIndex][i], house.lastProbs[i])
                            if i == 0:
                                relevantEvents = [x for x in hospitalizationEvents if x.age <= member.age]
                                if len(relevantEvents) > 0:
                                    house.kindOfEvent = 'hospitalization'
                            elif i == 1:
                                relevantEvents = [x for x in intubationEvents if x.age <= member.age]
                                if len(relevantEvents) > 0:
                                    house.kindOfEvent = 'intubation'
                            elif i == 2:
                                relevantEvents = [x for x in deathEvents if x.age <= member.age]
                                if len(relevantEvents) > 0:
                                    house.kindOfEvent = 'death'
                            if len(relevantEvents) > 0:
                                
                                house.experiencedEvent = True
                                
                                surpriseFactor = 0
                                for event in relevantEvents:
                                    surpriseFactor += np.power(self.p['conditionCostFactor'], i)*event.distanceWeight
                                    
                                SF.append(surpriseFactor)    
                                    
                                expEvent = np.exp(self.p['eventsBeta']*surpriseFactor)
                                gamma = (expEvent-1.0)/expEvent
                                perceivedProb = baseProb+gamma*(1.0-baseProb)
                            else:
                                perceivedProb = baseProb-self.p['biasExtintionRate']*(baseProb-self.probsConditionsByAge[ageIndex][i])
                            expectedCost += np.power(self.p['conditionCostFactor'], i)*perceivedProb
                            house.lastProbs[i] = perceivedProb
                        if expectedCost > highestExpectedCost:
                            highestExpectedCost = expectedCost
                    house.expectedCost = highestExpectedCost
                    
                    C.append(house.expectedCost)
                    
                    # Compute the probability component of the isolation propensity.
                    # First, the beta needs to be computed.
                    # It depends on the presence of one or more 'infection' events.
                    # If a neighbor is infected whih was relatively isolated, beta is adjusted upwards.
                    infectionEvents = [x for x in house.periodEvents if x.condition == 'infected' or x.condition == 'intubated' or x.condition == 'hospitalized']
                    maxIsolationRate = 0.0
                    betaFactor = 0
                    if len(infectionEvents) > 0:
                        house.infectionEvent = True
                        maxIsolationRate = max([x.meanIsolationRate for x in infectionEvents])
                        relevantEventWeight = [x.distanceWeight for x in infectionEvents if x.meanIsolationRate == maxIsolationRate][0]
                        betaFactor = (house.meanIsolationRate-maxIsolationRate)*relevantEventWeight
                        if maxIsolationRate > house.meanIsolationRate:
                            house.beta *= np.exp(self.p['betaSurpriseExp']*(maxIsolationRate-house.meanIsolationRate)*relevantEventWeight)
                    elif len(infectionEvents) == 0 or maxIsolationRate < house.meanIsolationRate:
                        house.beta *= np.exp(self.p['betaExtintionExp']*(self.p['initialBeta']-house.beta))
                        if house.beta < self.p['initialBeta']:
                            house.beta = self.p['initialBeta']
                        
                    BF.append(betaFactor)
                    
                    house.contactReductionRate = 0
                    
                    dens = []
                    for x in self.p['venuesSizesList']:
                        attendanceFactor = math.pow(x, self.p['venueBetaExp'])
                        dens.append(np.power(1.0+self.p['infectionProbFactor']/(np.exp(house.beta*self.newCasesRatio*attendanceFactor)-1.0), self.p['infectionProbExp']))
                    attendanceFactor = math.pow(100, self.p['venueBetaExp'])
                    dens.append(np.power(1.0+self.p['infectionProbFactor']/(np.exp(house.beta*self.newCasesRatio*attendanceFactor)-1.0), self.p['infectionProbExp']))
                    
                    Phi.append(np.exp(house.beta*self.newCasesRatio*attendanceFactor))
                    if Phi[-1] == 1.0:
                        print 'House beta: ' + str(house.beta)
                        print 'New cases ratio: ' + str(self.newCasesRatio)
                    Den.append(dens[-1])
                    
                    isolationFactors = [highestExpectedCost/x for x in dens]
                    
                    house.isolationFactor = isolationFactors[0]
                    
                    L.append(isolationFactors[10])
                    
                    house.baseIsolationPropensities = [(1.0-np.exp(-1*self.p['betaIsolationRate']*(house.incomeQuintile+1)*x)) for x in isolationFactors]
                    
                    house.individualIsolation = house.baseIsolationPropensities[0]
                else:
                    house.baseIsolationPropensities = [x*(1.0-self.p['isolationReducingRate']) for x in house.baseIsolationPropensities]
                
                
                # house.baseIsolationPropensities = [w*self.p['adaptationRate']+r*(1.0-self.p['adaptationRate']) for w, r in zip(house.baseIsolationPropensities, house.contactReductionRates)]
                
                # Compute the house contact reduction rate as a weighted average between the base isolation rates and the social isolation rates
                
                # house.contactReductionRates = [w*self.p['isolationWeight']+r*(1.0-self.p['isolationWeight']) for w, r in zip(socialIsolations, house.baseIsolationPropensities)]
                
                # Adust the contact reduction rate for the pandemic events
    #            hospitalizationEvents = [x for x in house.periodEvents if x.condition == 'hospitalized']
    #            intubationEvents = [x for x in house.periodEvents if x.condition == 'intubated']
    #            deathEvents = [x for x in house.periodEvents if x.condition == 'death'] 
    #            totalCostHosp = 0
    #            for event in hospitalizationEvents:
    #                totalCostHosp += np.power(self.p['conditionCostFactor'], 0)*event.distanceWeight
    #            # totalCostHosp = np.power(totalCostHosp, self.p['eventCostExp'])
    #            totalCostInt = 0
    #            for event in intubationEvents:
    #                totalCostInt += np.power(self.p['conditionCostFactor'], 1)*event.distanceWeight
    #            # totalCostInt = np.power(totalCostInt, self.p['eventCostExp'])
    #            totalCostDea = 0
    #            for event in deathEvents:
    #                totalCostDea += np.power(self.p['conditionCostFactor'], 2)*event.distanceWeight
    #            # totalCostDea = np.power(totalCostDea, self.p['eventCostExp'])
    #            expEvent = np.exp(self.p['eventsBeta']*(totalCostHosp+totalCostInt+totalCostDea))
    #            gamma = (expEvent-1.0)/expEvent
    #            house.contactReductionRates = [(x+gamma*(1.0-x)) for x in house.contactReductionRates]
        ...
        for house in self.map.occupiedHouses:
            ...
            socialIsolations = []
            for i in range(int(self.p['isolationRatesNum'])):
                isolationRates = [x.baseIsolationPropensities[i] for x in neighbors]
                socialIsolations.append(sum([w*r for w, r in zip(weights, isolationRates)]))
            
            house.neighborIsolation = socialIsolations[0]
            
            
            house.contactReductionRates = [w*self.p['isolationWeight']+r*(1.0-self.p['isolationWeight']) for w, r in zip(house.baseIsolationPropensities, socialIsolations)]  
            
            house.contactReductionRate = house.contactReductionRates[0]
            house.meanIsolationRate = sum([w*r for w, r in zip(house.contactReductionRates, self.shareAttendances)])
        
        if len(L) > 0:
            print 'max L: ' + str(max(L))
            print 'min L: ' + str(min(L))
            print 'Median L: ' + str(np.median(L))
        if len(C) > 0:
            print 'max C: ' + str(max(C))
            print 'min C: ' + str(min(C))
            print 'Median C: ' + str(np.median(C))
        if len(Phi) > 0:
            print 'max Phi: ' + str(max(Phi))
            print 'min Phi: ' + str(min(Phi))
            print 'Median Phi: ' + str(np.median(Phi))
        if len(SF) > 0:
            print 'max SF: ' + str(max(SF))
            print 'min SF: ' + str(min(SF))
            print 'Median SF: ' + str(np.median(SF))
        if len(BF) > 0:
            print 'max BF: ' + str(max(BF))
            print 'min BF: ' + str(min(BF))
            print 'Median BF: ' + str(np.median(BF))
        if len(Den) > 0:
            print 'max Den: ' + str(max(Den))
            print 'min Den: ' + str(min(Den))
            print 'Median Den: ' + str(np.median(Den))
...
#                meanNumContacts = float(averageContacts)*agent.contactReductionRate*averageIsolationRate
        ...
        # At this stage, the agents' daily contacts are determined
        # List of agents with additional contacts: 
        
        # Subset of C who have available contacts
        # While len(C) > 0:
        # Select a random agent and a random friend
        # Reduce contacts of the two agents
        # Determine if contact takes place
        # If yes, update the two agents' daily contacts
...
#            self.daysToLockdown -= 1
        ...
        for agent in infected:
            ...
            if agent.healthStatus == 'exposed' and agent.daysFromInfection >= (agent.incubationPeriod-self.p['preSymptomsContagiousPeriod']):
                ...
                if agent.symptomsLevel == 'severe' or agent.symptomsLevel == 'critical' or agent.placeOfDeath == 'Hospital' or agent.placeOfDeath == 'ICU':
                    ...
                    if agent.symptomsLevel == 'severe' or agent.placeOfDeath == 'Hospital':
                        ...
                        for neighbor in neighbors:
                            ...
                            newEvent = PandemicEvent(agent.age, 'hospitalized', agent.house.contactReductionRates, meanIsolationRate, weight)
                    ...
                    if agent.symptomsLevel == 'critical' or agent.placeOfDeath == 'ICU':
                        ...
                        for neighbor in neighbors:
                            weight = neighbor.infoNetwork[neighbor][agent.house]['weight']
                            meanIsolationRate = sum([w*r for w, r in zip(agent.house.contactReductionRates, self.shareAttendances)])
                            newEvent = PandemicEvent(agent.age, 'intubated', agent.house.contactReductionRates, meanIsolationRate, weight)
                            neighbor.periodEvents.append(newEvent)
                ...
                elif agent.placeOfDeath == 'Home':
                    
                    neighbors = [x for x in agent.house.infoNetwork.neighbors(agent.house)]
                    for neighbor in neighbors:
                        weight = neighbor.infoNetwork[neighbor][agent.house]['weight']
                        meanIsolationRate = sum([w*r for w, r in zip(agent.house.contactReductionRates, self.shareAttendances)])
                        newEvent = PandemicEvent(agent.age, 'infected', agent.house.contactReductionRates, meanIsolationRate, weight)
                        neighbor.periodEvents.append(newEvent)
                ...
                if agent.symptomsLevel == 'dead':
                    ...
                    for neighbor in neighbors:
                        weight = neighbor.infoNetwork[neighbor][agent.house]['weight']
                        meanIsolationRate = sum([w*r for w, r in zip(agent.house.contactReductionRates, self.shareAttendances)])
                        newEvent = PandemicEvent(agent.age, 'death', agent.house.contactReductionRates, meanIsolationRate, weight)
                        neighbor.periodEvents.append(newEvent)
        ...
        for person in noTestMildAgents:
            ...
            if np.random.random() < probTest:
                ...
                for neighbor in neighbors:
                    ...
                    meanIsolationRate = sum([w*r for w, r in zip(person.house.contactReductionRates, self.shareAttendances)])
                    newEvent = PandemicEvent(person.age, 'infected', person.house.contactReductionRates, meanIsolationRate, weight)
                    neighbor.periodEvents.append(newEvent)
                ...
                for member in household:
                    ...
                    if member.father != None:
                        ...
                        if nok.dead == False and nok not in household and nok.house not in existingNodes:
                            distance = self.geoDistance(house, nok.house)
                            distanceFactor = self.p['locationInteractionBeta']*math.pow(distance, self.p['locationInteractionExp'])
                            contactWeight = 1.0/(1.0+distanceFactor)
                            house.infoNetwork.add_edge(house, nok.house, weight = contactWeight)
                            nok.house.infoNetwork.add_edge(nok.house, house, weight = contactWeight)
...
#        print 'Unmet child care need: ' + str(unmetChildCareNeed)
        ...
        outputs = [day, currentPop, everLivedPop, numHouseholds, averageHouseholdSize, self.marriageTally, marriagePropNow, 
                   ...
                   self.mobilityReduction_A7, self.mobilityReduction_A8, self.mobilityReduction_A9, self.totalAttendants, self.cumulatedAbsences]
